...
void noonFunc(float time, float timeFactor) {
    if(isBiomeEnd) {
        ...
        fogMin = FOG_DAY_DIST_MIN; //mix2(baseFogDistMin, FOG_DAY_DIST_MIN, dayNightLerp);
        fogMax = FOG_DAY_DIST_MAX; //mix2(baseFogDistMax, FOG_DAY_DIST_MAX, dayNightLerp);
        ...
        fogAlbedo = vec3(FOG_DAY_R, FOG_DAY_G, FOG_DAY_B); //mix2(baseFog, vec3(FOG_DAY_R, FOG_DAY_G, FOG_DAY_B), dayNightLerp);
    }
}
...
void sunsetFunc(float time, float timeFactor) {
    if(isBiomeEnd) {
        ...
        fogMin = mix3(FOG_DAY_DIST_MIN,FOG_SUNSET_DIST_MIN,FOG_NIGHT_DIST_MIN, sunsetLerp, 0.5); //mix2(baseFogDistMin, FOG_DAY_DIST_MIN, dayNightLerp);
        fogMax = mix3(FOG_DAY_DIST_MAX,FOG_SUNSET_DIST_MAX,FOG_NIGHT_DIST_MAX, sunsetLerp, 0.5); //mix2(baseFogDistMax, FOG_DAY_DIST_MAX, dayNightLerp);
        ...
        Diffuse = pow2(Diffuse.rgb,vec3(GAMMA)) * baseDiffuseModifier; //mix2(baseDiffuse, pow2(Diffuse.rgb,vec3(GAMMA)) * baseDiffuseModifier, mod(worldTime/6000f,2f));
        fogAlbedo = mix3(vec3(FOG_DAY_R, FOG_DAY_G, FOG_DAY_B), vec3(FOG_SUNSET_R, FOG_SUNSET_G, FOG_SUNSET_B), vec3(FOG_NIGHT_R, FOG_NIGHT_G, FOG_NIGHT_B), sunsetLerp, 0.5); //mix2(baseFog, vec3(FOG_SUNSET_R, FOG_SUNSET_G, FOG_SUNSET_B), sunsetLerp);
    }
}
...
void nightFunc(float time, float timeFactor) {
    if(isBiomeEnd) {
        ...
        mediump float dayNightLerp = clamp((time+250f)/timeFactor,0,1);
        fogMin = FOG_NIGHT_DIST_MIN; //mix2(baseFogDistMin, FOG_NIGHT_DIST_MIN, dayNightLerp);
        fogMax = FOG_NIGHT_DIST_MAX; //mix2(baseFogDistMax, FOG_NIGHT_DIST_MAX, dayNightLerp);
        baseDiffuseModifier = vec3(NIGHT_I * 0.4f);
        currentColor = nightColor; //mix2(baseColor, nightColor, dayNightLerp);
        Diffuse = pow2(Diffuse.rgb,vec3(GAMMA)) * baseDiffuseModifier; //mix2(baseDiffuse, pow2(Diffuse.rgb,vec3(GAMMA)) * baseDiffuseModifier,mod(worldTime/6000f,2f));
        fogAlbedo = vec3(FOG_NIGHT_R, FOG_NIGHT_G, FOG_NIGHT_B); //mix2(baseFog, vec3(FOG_NIGHT_R, FOG_NIGHT_G, FOG_NIGHT_B), dayNightLerp);
    }
}
...
void dawnFunc(float time, float timeFactor) {
    if(isBiomeEnd) {
        ...
        mediump float sunsetLerp = clamp((time)/timeFactor,0,1);
        fogMin = mix3(FOG_NIGHT_DIST_MIN,FOG_SUNSET_DIST_MIN,FOG_DAY_DIST_MIN, sunsetLerp, 0.5); //mix2(baseFogDistMin, FOG_DAY_DIST_MIN, dayNightLerp);
        fogMax = mix3(FOG_NIGHT_DIST_MAX,FOG_SUNSET_DIST_MAX,FOG_DAY_DIST_MAX, sunsetLerp, 0.5); //mix2(baseFogDistMax, FOG_DAY_DIST_MAX, dayNightLerp);
        ...
        Diffuse = pow2(Diffuse.rgb,vec3(GAMMA)) * baseDiffuseModifier; //mix2(baseDiffuse, pow2(Diffuse.rgb,vec3(GAMMA)) * baseDiffuseModifier, mod(worldTime/6000f,2f));
        fogAlbedo = mix3(vec3(FOG_NIGHT_R, FOG_NIGHT_G, FOG_NIGHT_B), vec3(FOG_SUNSET_R, FOG_SUNSET_G, FOG_SUNSET_B), vec3(FOG_DAY_R, FOG_DAY_G, FOG_DAY_B), sunsetLerp, 0.5); //mix2(baseFog, vec3(FOG_SUNSET_R, FOG_SUNSET_G, FOG_SUNSET_B), sunsetLerp);
    }
}
