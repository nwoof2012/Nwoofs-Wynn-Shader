    ...
    vec3 GetSunVector() {
        ...
        #ifdef OVERWORLD
            mediump float ang = fract(timeAngle - 0.25);
            ang = (ang + (cos(ang * 3.14159265358979) * -0.5 + 0.5 - ang) / 3.0) * 6.28318530717959;
            return normalize2((gbufferModelView * vec4(vec3(-sin(ang), cos(ang) * sunRotationData) * 2000.0, 1.0)).xyz);
        #elif defined END
            mediump float ang = 0.0;
            return normalize2((gbufferModelView * vec4(vec3(0.0, sunRotationData * 2000.0), 1.0)).xyz);
        #else
            return vec3(0.0);
        #endif
    }
...
mediump float GetLuminance(vec3 color) {
...
mediump float GetBiasFactor(float NdotLM) {
    mediump float NdotLM2 = NdotLM * NdotLM;
...
mediump float GetMaxColorDif(vec3 color) {
...
mediump float min1(float x) {
    return min(x, 1.0);
}
int max0(int x) {
    return max(x, 0);
}
mediump float max0(float x) {
    return max(x, 0.0);
}
int clamp01(int x) {
    return clamp(x, 0, 1);
}
mediump float clamp01(float x) {
...
mediump float pow1_5(float x) { // Faster pow(x, 1.5) approximation (that isn't accurate at all) if x is between 0 and 1
...
mediump float sqrt1(float x) { // Faster sqrt() approximation (that isn't accurate at all) if x is between 0 and 1
