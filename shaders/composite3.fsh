...
vec4 cloudStack(in vec2 uv) {
    ...
    for(float i = 0; i < cloudLevels; i++) {
        ...
        noiseA = mix(pow(noiseA, vec4(CLOUD_DENSITY)),pow(noiseA, vec4(CLOUD_DENSITY_RAIN)),rainStrength);
        noiseB = mix(pow(noiseA, vec4(CLOUD_DENSITY)),pow(noiseB, vec4(CLOUD_DENSITY_RAIN)),rainStrength);
    ...
    //finalNoise.a = clamp(pow(finalNoise.a, 0.9)/(cloudLevels*cloudLevels),0,1);
...
void main() {
    ...
    if(depth == 1.0) {
        ...
        noiseA.rgb = mix(pow(noiseA.rgb, vec3(CLOUD_DENSITY)),pow(noiseA.rgb, vec3(CLOUD_DENSITY_RAIN)),rainStrength);
        noiseB.rgb = mix(pow(noiseA.rgb, vec3(CLOUD_DENSITY)),pow(noiseB.rgb, vec3(CLOUD_DENSITY_RAIN)),rainStrength);
        ...
        /*if(rayDir.y > 0)
        {
            ...
            for(float s = 0.0; s < samples; s++) {
                ...
                finalNoise.rgb = mix(finalNoise.rgb, cloud.rgb,(1 - finalNoise.a) * cloud.a);
        ...
        noiseNormalA = mix(pow(noiseA, vec4(CLOUD_DENSITY)),pow(noiseA, vec4(CLOUD_DENSITY_RAIN)),rainStrength);
        noiseNormalB = mix(pow(noiseA, vec4(CLOUD_DENSITY)),pow(noiseB, vec4(CLOUD_DENSITY_RAIN)),rainStrength);
        ...
        clouds.rgb *= mix(75.5,1.0, rainStrength);
        //clouds.a = (clouds.a - 0.125) * 1.0;
        /*if(clouds.a - 3.45 > 0.0) {
            clouds.a = pow(clouds.a - 3.45,4.0) + 3.45;
        } else if(clouds.a - 3.45 < 0.0) {
            clouds.a = pow(clouds.a - 3.45,1/4.0) + 3.45;
        }*/
        ...
        if(rayDir.y > 0.0 || detectSky != 1.0) {
            color.rgb = mix(color.rgb, clouds.rgb,clouds.a*(cloudFog * CLOUD_FOG));
        }
